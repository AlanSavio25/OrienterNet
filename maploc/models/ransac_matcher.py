...
def sample_transforms_ransac(
    ...
    bev_ij_pts_pool = (
        bev_ij_pts.reshape(-1, 2)[indices[..., 0]].reshape(pool_shape).float()
    )
    map_ij_pts_pool = indices[..., 1:].reshape(pool_shape).float()

    if num_retries > 1:
        # sample multiple minimal sets and retain those that are most consistent
        d_bev = torch.linalg.norm(
            torch.diff(bev_ij_pts_pool, dim=-2).squeeze(-2), dim=-1
        )  # [num_poses, num_retries]
        d_map = torch.linalg.norm(
            torch.diff(map_ij_pts_pool, dim=-2).squeeze(-2), dim=-1
        )  # [num_poses, num_retries]
        ratio = torch.maximum(
            d_bev / d_map.clamp(min=1e-5), d_map / d_bev.clamp(min=1e-5)
        )  # [num_poses, num_retries]
        select_indices = torch.argmin(ratio, dim=-1)
        select_fn = torch.vmap(
            lambda x, i: x[i[None]][0]
        )  # work-around for .item() error
...
def grid_refinement(
    ...
    scores = pose_scoring_many(
        map_T_cam_samples._data,  # vmap does not support TensorWrappers
        sim_points,
        f_bev,
        valid_bev,
        valid_map,
...
pose_scoring_many_batched = torch.vmap(
    pose_scoring_many, in_dims=(0,) * 2 + (None,) + (0,) * 2
)
grid_refinement_batched = torch.vmap(
    grid_refinement, in_dims=(0,) * 2 + (None,) * 2 + (0,) * 1
)
